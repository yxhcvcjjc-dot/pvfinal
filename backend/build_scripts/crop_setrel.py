"""Crop Set & Relation question screenshots into question + 4 option images.
Reuses the Matrices detection logic (same app, same fixed layout)."""
...
BANK = "/app/backend/chapter_banks/neet-math-set-and-relation.json"
PREFIX = "setrel"

PLAN = [
    ("Questions on number of relations and sets", 16),
    ("Questions on Venn Diagram", 2),
]
...
def main(mode="analyze"):
    ...
    sections = {t: [] for t, _ in PLAN}
    problems = []
    for i, f in enumerate(files):
        qno = i + 1
        (W, H), bands = detect_bands(os.path.join(SRC, f))
        if len(bands) < 5:
            problems.append((qno, f, len(bands), bands)); print(f"[FAIL q{qno}] {f} nb={len(bands)} {bands}"); continue
        q = bands[0]; opts = bands[1:5]
        if mode == "analyze":
            print(f"[q{qno}] {order[i][:34]:34s} nb={len(bands)} q={q} opts={opts}"); continue
